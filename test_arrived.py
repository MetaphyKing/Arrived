...
class ArrivedV2(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_seq_command(self) -> None:
        ...
        self.assertIn("ARRIVED id=42 hop2=command", r.stdout)
        self.assertNotIn("[OK]", r.stdout)

    def test_seq_file(self) -> None:
        hop1 = self.py("hop1.py", "print('posted id=42')\n")
        ...
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
        self.assertIn("hop2=file", r.stdout)
    ...
    def test_stream_file_stale_id(self) -> None:
        remote = self.dir / "far.log"
    ...
    def test_usage_neither_remote(self) -> None:
        hop1 = self.py("hop1.py", "print('posted id=1')\n")
        r = invoke(["--local-cmd", cmdline(PY, hop1)])
